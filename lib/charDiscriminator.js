...
function charDiscriminator (uuid) {
...
ValObj.prototype.getCharValParser = function (bBuf, uuid) {
    ...
    _.forEach(charVals.params, function (param, idx) {
        ...
        if ( valType === 'string' ) {
            if (bBuf[bBuf.length - 1] === 0) bBuf = bBuf.slice(0, bBuf.length - 1);
            ...
            chunkRules.push(ru.list(param, charVals.objInfo, bBuf.length));
        } else if (valType === 'buffer') {
            chunkRules.push(ru[valType](param, bBuf.length - charVals.preBufLen));
        } else {
    ...
    if (charVals.depends) {
        chunkRules.push(ru.depends(charVals.depends.params, charVals.depends.types));
    }
...
ValObj.prototype.getCharValPacket = function (bBuf, callback) {
    ...
    if (!this.uuid) {
        ...
        _.forEach(this.uuid, function (uuid, hdl) {
            rules.push(ru.squash(hdl, self.getCharValParser(bBuf, uuid)));
        });
        ...
        if (bufLen && (bBuf.length !== bufLen) && 
            (_.isArray(bufLen) && !_.includes(bufLen, bBuf.length)))
        {
            callback(null, bBuf);
        } else {
...
ru.clause('depends', function (extParams, extTypes) {
    var self = this;
    ...
    this.tap(function () {
        ...
        _.forEach(extParams, function (param, i) {
            if ((flags & num) === num) 
                ru[extTypes[i]](param)(self);
...
module.exports = charDiscriminator;
