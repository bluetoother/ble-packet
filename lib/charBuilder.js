...
var _ = require('busyman'),
    ...
    charMeta = require('./defs/charMeta');

var nibbleBuf = [];

function charBuilder (uuid) {
...
ValObj.prototype.transToValObj = function (argInst) {
    ...
    _.forEach(charVals.params, function (param) {
        if (!_.has(argInst, param)) { throw new Error('The argument object has incorrect properties.'); }
        self[param] = argInst[param];
    });

    if (charVals.extra) {
        if (_.has(argInst, 'flags')) {
            extJudge = function(i) { return ((argInst.flags & charVals.extra.flags[i]) === charVals.extra.result[i]); };
        } else if (_.has(argInst, 'condition')) {
        ...
        for(var i = 0; i < _.size(charVals.extra.params); i += 1) {
            if (extJudge(i)) {
                ...
                if (_.has(argInst, 'condition')) break;
            }
        }
    }
    ...
    if (charVals.depends) {
        ...
        _.forEach(charVals.depends.params, function (param, i) {
            if ((self.flags & num) === num) 
                self[param] = argInst[param];
...
ValObj.prototype.getCharBuf = function (callback) {
    var self = this,
        dataBuf = Concentrate(),
        charVals;
    ...
    if (charVals.extra) {
        if (_.has(this, 'flags') || _.has(this, 'condition') || _.has(this, 'opcode')) {
            _.forEach(charVals.extra.params, function (param, idx) {
                if (_.has(self, param)) builder(dataBuf, self[param], charVals.extra.types[idx], charVals);
            });
        }
    }
    ...
    if (charVals.depends) {
        _.forEach(charVals.depends.params, function (param, i) {
            if (_.has(self, param)) {
                builder(dataBuf, self[param], charVals.depends.types[i], charVals);
            }
        });
    }
...
function builder(dataBuf, paramVal, paramType, charVals) {
    ...
    switch (paramType) {
        ...
        case 'stringPreLenUint8':
            dataBuf = dataBuf.uint8(paramVal.length).string(paramVal, 'utf8');
            break;
        case 'bufferPreLenUint8':
            dataBuf = dataBuf.uint8(paramVal.length).buffer(paramVal);
            break;
        case 'list':
