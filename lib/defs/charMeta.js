...
var _ = require('lodash'),
    ipsoChar = require('ipso-ble-char');
...
_.forEach(ipsoCharUuids, function (uuid) {
    var newUuid = '0x' + uuid.toString(16),
        charParam = ipsoChar.getParams(uuid),
        paramObj = {
            params: [],
            types: [],
            depends: {
    ...
    _.forEach(charParam.mandatory, function (entry) {
        paramObj.params.push(entry.name);
        paramObj.types.push(transType(entry.type));
    });
    _.forEach(charParam.optional, function (entry) {
